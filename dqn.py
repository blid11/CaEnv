...
writer = SummaryWriter("dqn_data")

# cd PycharmProjects\Timed3  tensorboard --logdir dqn_data
...
def world_dynamics(t, replay_start_size, num_actions, s, env, policy_net):
    ...
    if t < replay_start_size:
        ...
        # #goodish one
        # if (t - replay_start_size >= FIRST_N_FRAMES) and (t < 3000000):
        #     epsilon = END_EPSILON
        # elif (t >= 3000000) and (t < 3900000):
        #     # decay to zero for 1 million frames
        #     epsilon = 0.1 - 0.0000001*(t - 3000000)
        # elif t >= 3900000:
        #     epsilon = 0.01
        # else:
        #     epsilon = ((END_EPSILON - EPSILON) / FIRST_N_FRAMES) * (t - replay_start_size) + EPSILON

        # epsilon =  0.01**(t/5000000)
...
def train(sample, policy_net, target_net, optimizer):
    ...
    policy_net.batch_numberr += 1

    if policy_net.batch_numberr % 50 == 0:
        writer.add_scalar("Loss/train", loss, policy_net.batch_numberr)
...
def dqn(env, replay_off, target_off, output_file_name, store_intermediate_result=False, load_path=None, step_size=STEP_SIZE):
    ...
    while t < NUM_FRAMES:
        ...
        while(not is_terminated) and t < NUM_FRAMES:
            ...
            if is_terminated:
                print("episode is {}, return is {}".format(e, G))
        ...
        if e % 20 == 0:
            # writer.add_scalar("Return", str(G), str(e))
            # writer.add_scalar("Avg/return", str(numpy.around(avg_return, 2)), str(e))
        ...
        if policy_net.batch_numberr % 100 == 0:
            writer.add_scalar("Return", G, e)
            writer.add_scalar("Avg Return", numpy.around(avg_return, 2), e)
...
def main():
    ...
    # parser.add_argument("--data_1_data_and_weights", "-o", type=str)
    ...
    dqn(env, False, args.targetoff, file_name, args.save, load_file_path, args.alpha)
